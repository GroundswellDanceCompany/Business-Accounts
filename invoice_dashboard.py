...
st.set_page_config(page_title="Dance School Manager", layout="wide")
...
if selection == "Invoice Generator":
    ...
    if st.button("Generate Invoice"):
        ...
        st.success(f"Invoice created for {student} (Â£{grand_total:.2f})")
    
        whatsapp_msg = (
            f"Hi {student}, your invoice for the period {invoice_period} is ready.\n"
            f"Total: Â£{grand_total:.2f}\n"
            f"Classes: {class_names} ({classes_attended} at Â£{total_class_rate}/class)"
        )
        if st.session_state.extras:
            whatsapp_msg += f"\nExtras: {extra_names}"
        whatsapp_msg += "\nThank you!"
